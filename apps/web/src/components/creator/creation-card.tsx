...
export function CreationCard({ item, onClick, onDelete }: CreationCardProps) {
  ...
  const statusBadge = isCompleted ? (
    <Badge className="bg-[#b6ff00]/25 text-[#5c8200] hover:bg-[#b6ff00]/25">{t("creator.statusCompleted")}</Badge>
  ) : isFailed ? (
    <Badge
      variant="outline"
  ...
  return (
    <div className="group relative w-full">
      ...
      <button
        type="button"
        onClick={onClick}
        className="w-full overflow-hidden rounded-xl border border-[#e5e7eb] bg-white text-left transition hover:-translate-y-0.5 hover:border-[#cbd0d6] hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#8bc400]/40"
      >
        <div className="relative isolate aspect-[4/3] overflow-hidden bg-[#f1f3f5]">
          ...
          {isFailed ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-2">
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-[#8bc400]/25 border-t-[#8bc400]" />
