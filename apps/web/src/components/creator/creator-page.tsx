...
import type { AspectRatio, CreationStats, MediaQuality } from "@printfilm/shared-types";
...
export function CreatorPage() {
  ...
  const pillClass = (active: boolean) =>
    active
      ? "rounded-full border border-[#b6ff00] bg-[#f7ffe8] px-3.5 py-1.5 text-sm font-medium text-[#111318]"
      : "rounded-full border border-[#e5e7eb] bg-white px-3.5 py-1.5 text-sm font-medium text-[#111318] transition hover:border-[#cbd0d6] hover:bg-[#f7f8fa]";
    ...
    <div className="pf-shell-main space-y-8">
      <header className="pf-page-head">
        <div className="pf-page-head-row">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              className="bg-[#b6ff00] text-[#111318] shadow-none hover:bg-[#a8f000]"
              onClick={() => requireAuthFor("/projects?entry=dramaforge")}
            >
              <Sparkles />
      ...
      <section>
        <div className="mb-3 flex items-end justify-between gap-3">
          <h2 className="text-base font-semibold text-[#111318]">{t("creator.statsTitle")}</h2>
        </div>
            ...
            return (
              <div key={stat.label} className="rounded-xl border border-[#e5e7eb] bg-white p-4">
                <div className="flex items-start justify-between">
                  ...
                  <span className="grid size-9 place-items-center rounded-lg bg-[#efffc7] text-[#466400]">
                    <Icon className="size-4" />
                  </span>
                </div>
                <p className="mt-2 text-2xl font-bold tracking-tight text-[#111318]">{stat.value}</p>
                <p className={`mt-1 text-xs font-medium ${stat.up ? "text-[#6f9d00]" : "text-rose-600"}`}>
                  {stat.delta}
                </p>
              </div>
            );
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold tracking-tight text-[#111318]">{t("creator.quickAccess")}</h2>
          ...
          {featureCards.map((card) => (
            <button
              ...
              className={`overflow-hidden rounded-xl border bg-white text-left transition hover:-translate-y-0.5 hover:border-[#cbd0d6] hover:shadow-sm ${
                card.selected
                  ? "border-[#b6ff00] shadow-[0_0_0_2px_rgba(182,255,0,0.35)]"
                  : "border-[#e5e7eb]"
              }`}
            >
              <div className="flex aspect-[16/9] items-end bg-[#111318] p-4">
                <span className="rounded-md bg-[#b6ff00] px-2 py-0.5 text-[11px] font-semibold text-[#111318]">
                  {card.title}
                </span>
              </div>
      ...
      <section id="creation-workbench" className="scroll-mt-20 overflow-hidden rounded-xl border border-[#e5e7eb] bg-white">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#e5e7eb] px-5 py-4 md:px-6">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-bold text-[#111318]">
              <WandSparkles className="size-5 text-[#8bc400]" />
              {t("creator.workbench")}
            </h2>
            <p className="mt-1 text-sm text-[#62666d]">{t("creator.workbenchHint")}</p>
          </div>
        ...
        <div className="space-y-4 p-5 md:p-6">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Select value={form.model} onValueChange={(model) => updateForm({ model })} disabled={loadingModels || models.length === 0}>
              <SelectTrigger className="min-w-[190px] bg-[#f7f8fa]">
            ...
            {(form.mode === "video" || form.mode === "image") && (
              <>
                <Select value={form.ratio} onValueChange={(ratio) => updateForm({ ratio: ratio as AspectRatio })}>
                  <SelectTrigger className="w-[120px] bg-[#f7f8fa]">
          ...
          <textarea
            ...
            className="w-full resize-none rounded-xl border border-[#e5e7eb] bg-[#f7f8fa] p-4 text-sm leading-relaxed outline-none transition placeholder:text-[#858a92] focus:border-[#9bd900] focus:ring-4 focus:ring-[#b6ff00]/20"
          />
          ...
          {needsReference && (
            <div>
              ...
              <Button
                ...
                className="h-20 w-full border-dashed bg-[#f7f8fa] text-[#62666d]"
              >
                <Upload />
                {form.referenceUrl
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                className="min-w-44 bg-[#b6ff00] font-semibold text-[#111318] shadow-none hover:bg-[#a8f000]"
              >
                <WandSparkles />
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h2 className="text-xl font-bold tracking-tight text-[#111318]">{t("creator.myCreations")}</h2>
          </div>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-[#e5e7eb] bg-white p-5">
          <div className="mb-4 flex items-center justify-between gap-2">
            <h2 className="text-base font-semibold text-[#111318]">{t("creator.announcements")}</h2>
          ...
          <div className="space-y-3">
            {[
              ...
              <div
                ...
                className="rounded-lg border border-transparent p-2 transition hover:border-[#e5e7eb] hover:bg-[#f7f8fa]"
              >
                <div className="flex items-center justify-between gap-2">
                  <Badge variant="secondary">{announcement.tag}</Badge>
                  <span className="text-[10px] text-[#858a92]">{announcement.date}</span>
                </div>
                <p className="mt-2 text-sm text-[#111318]">{announcement.title}</p>
              </div>
            ))}
          </div>
        </div>
