import type { GenerationJobRecord, ImageSubMode, VideoSubMode } from "@printfilm/shared-types";
...
export function getDownloadFilename(item: CreationItem) {
  ...
  if (item.mode === "video") return `printfilm-${base}.mp4`;
  if (item.mode === "image") {
    const ext = item.outputUrl?.includes(".png") ? "png" : item.outputUrl?.includes(".webp") ? "webp" : "jpg";
    return `printfilm-${base}.${ext}`;
  }
  return `printfilm-${base}.txt`;
}
