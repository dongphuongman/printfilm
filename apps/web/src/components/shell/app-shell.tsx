    ...
    <div className={cn("flex items-center gap-2", compact && "w-full flex-wrap")}>
      ...
      {user && !compact && (
        <Badge
          ...
          className={
            apiConnected
              ? "hidden border-[#8bc400]/30 bg-[#b6ff00]/15 text-[#5c8200] sm:inline-flex"
              : "hidden border-amber-500/25 bg-amber-500/10 text-amber-700 sm:inline-flex"
          }
        >
          <span className={`mr-1.5 size-1.5 rounded-full ${apiConnected ? "bg-[#8bc400]" : "bg-amber-500"}`} />
      ...
      {user ? (
          ...
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-9 gap-2 rounded-lg px-2">
              <Avatar className="size-7 border border-border">
                <AvatarFallback className="bg-[#efffc7] text-xs font-semibold text-[#466400]">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Button size="sm" asChild className="bg-[#b6ff00] font-semibold text-[#111318] shadow-none hover:bg-[#a8f000]">
    ...
    <header className="pf-nav">
      <div className="pf-nav-left">
        <Link href="/" className="flex items-center gap-2.5">
          <span className="relative flex size-8 shrink-0 items-center justify-center overflow-hidden rounded-lg bg-[#111318]">
            <Image
              src="/brand-icon.png"
              alt={`${APP_NAME} logo`}
              width={32}
          ...
          <span className="hidden text-sm font-extrabold tracking-wide text-[#111318] sm:inline">
            PRINTFILM
          </span>
        </Link>
        {studioMode && (
            ...
            {items.map((item) => (
              <Link
                ...
                className={cn(
                  ...
                  item.active ? "bg-[#f7ffe8] text-[#111318]" : "text-[var(--pf-muted)] hover:bg-[#f7f8fa]",
                )}
              >
                {item.label}
              </Link>
