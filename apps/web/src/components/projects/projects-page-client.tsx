...
import type { Project } from "@printfilm/shared-types";
    ...
    return (
      <div className="mx-auto grid max-w-[1440px] gap-4 p-4 md:p-6 lg:grid-cols-[280px_minmax(0,1fr)_280px]">
        <aside className="pf-create-col space-y-3">
          <Link href="/projects" className="text-xs text-[var(--pf-muted)] hover:text-[#111318]">
            {t("projects.backToList")}
          </Link>
          <div>
            <div className="aspect-video overflow-hidden rounded-xl bg-[#111318]" />
          ...
          <nav className="space-y-0.5">
            {PROJECT_MENU.map((item) => (
              <Link
                ...
                className="block rounded-xl px-3 py-2 text-sm text-[var(--pf-muted)] hover:bg-[#f7ffe8] hover:text-[#111318]"
              >
                {t(item.labelKey)}
              </Link>
            ))}
          </nav>
          <div className="rounded-xl border border-[var(--pf-line)] bg-[#f7f8fa] p-3">
            <div className="text-[10px] text-[var(--pf-muted)]">{t("projects.storagePlaceholder")}</div>
            <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-muted">
              <div className="h-full w-[43%] rounded-full bg-[#b6ff00]" />
            </div>
        ...
        <section className="pf-create-col space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-[200px_1fr]">
            <div className="aspect-video rounded-xl bg-[#111318]" />
            <div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Link
                  ...
                  onClick={() => setCurrentProjectId(viewed.id)}
                  className="inline-flex rounded-lg bg-[#b6ff00] px-4 py-2 text-sm font-semibold text-[#111318] hover:bg-[#a8f000]"
                >
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-[var(--pf-line)] bg-[#f7f8fa] p-4">
              <div className="text-xs text-[var(--pf-muted)]">{t("projects.projectProgress")}</div>
              <div className="mt-2 text-2xl font-semibold">—</div>
              <div className="mt-2 h-2 overflow-hidden rounded-full bg-muted">
                <div className="h-full w-3/4 rounded-full bg-[#b6ff00]" />
              </div>
            ...
            <div className="rounded-xl border border-[var(--pf-line)] bg-[#f7f8fa] p-4">
              ...
              <div className="mt-3 flex -space-x-2">
                {[
                  ...
                  <span
                    key={c}
                    className="flex h-8 w-8 items-center justify-center rounded-full border border-white bg-secondary text-[10px] text-[#111318]"
                  >
                    {c}
                  </span>
                ))}
          ...
          <div className="rounded-xl border border-[var(--pf-line)] p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <Link
                href={`/studio/${viewed.id}`}
                className="text-xs font-medium text-[#6f9d00] hover:underline"
              >
        ...
        <aside className="pf-create-col hidden space-y-3 xl:block">
          ...
          <Link
            href={`/studio/${viewed.id}`}
            className="mt-2 block rounded-lg bg-[#b6ff00] px-3 py-2.5 text-center text-sm font-semibold text-[#111318] hover:bg-[#a8f000]"
          >
      ...
      <PfPageHead
        ...
        actions={(
          <Button
            ...
            className="bg-[#b6ff00] font-semibold text-[#111318] shadow-none hover:bg-[#a8f000]"
          >
            ...
            return (
              <Card key={project.id} className="pf-template-card overflow-hidden border-0 py-0 shadow-none">
                <Link
                  ...
                  className="relative block aspect-video bg-[#111318]"
                >
                  <div className="absolute inset-0 grid place-items-center"><FolderKanban className="size-12 text-[#b6ff00]/60" /></div>
                ...
                <CardContent className="space-y-3 p-5">
                  <Link
                    href={`/projects?view=${project.id}`}
                    onClick={() => setCurrentProjectId(project.id)}
                    className="block truncate font-semibold hover:text-[#6f9d00]"
                  >
                    {project.name}
                  </Link>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    <Link
                      ...
                      onClick={() => setCurrentProjectId(project.id)}
                      className="inline-flex h-8 items-center justify-center gap-1.5 rounded-md bg-[#b6ff00] px-3 text-sm font-semibold text-[#111318] shadow-none transition-colors hover:bg-[#a8f000]"
                    >
                      {t("projects.workflow")}
