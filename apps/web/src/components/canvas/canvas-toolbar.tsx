...
import type { CanvasNodeType } from "@printfilm/shared-types";
  ...
  return (
    <header className="flex h-12 shrink-0 items-center justify-between gap-4 border-b border-white/70 bg-white/78 px-4 shadow-sm backdrop-blur-xl">
      ...
      <div className="flex items-center rounded-full border border-slate-200 bg-slate-100/80 p-0.5">
        <button
          type="button"
          className="rounded-full border border-[#e5e7eb] bg-white px-3.5 py-1 text-xs font-medium text-[#111318]"
        >
      ...
      <div className="flex items-center gap-2 shrink-0">
        <div className="hidden xl:flex items-center gap-1.5">
          {nodeOptions.map((opt) => (
            <button
              ...
              className="rounded-md border border-slate-200 bg-white px-2 py-1 text-[11px] text-slate-600 transition hover:border-[#8bc400] hover:bg-[#f4ffd6] hover:text-[#111318]"
            >
              + {opt.label}
            </button>
          ))}
        ...
        {onSyncFromPipeline && (
          <button
            ...
            className="rounded-md bg-[#b6ff00] px-2.5 py-1 text-[11px] font-semibold text-[#111318] transition hover:bg-[#a8f000] disabled:opacity-50"
          >
