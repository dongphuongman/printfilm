...
import {
  ...
  selectDramaForgeAssetCandidate,
...
export function DramaForgeWorkspace({
  ...
  }, [shots, shotSearch, pendingShotsOnly, shotSceneFilter, shotStatusFilter, wizardStep, jobs]);
  ...
  async function handleSaveTimeline(timeline: import("@printfilm/shared-types").DramaForgeTimeline) {
    if (!selectedEpisodeId) return;
    await runAction(async () => {
      await updateDramaForgeEpisode(projectId, selectedEpisodeId, {
    ...
    <div className="overflow-hidden rounded-xl border border-[var(--ar-hairline)] bg-white/60">
      <div className="relative aspect-[4/3] bg-[#111318]">
        {imageUrl ? (
          <button
            type="button"
            className="h-full w-full cursor-zoom-in text-left transition hover:opacity-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#8bc400]/50"
        ...
        <div className="absolute left-2 top-2 flex flex-wrap gap-1">
          ...
          {bindingTag ? (
            <span className="rounded-full bg-[var(--ar-accent)]/90 px-2 py-0.5 text-[10px] text-[#111318]">
              {bindingTag}
            </span>
          ) : (
          ...
          {audioTag && (
            <span className="rounded-full bg-[#b6ff00]/90 px-2 py-0.5 text-[10px] text-[#111318]">
              {audioTag}
            </span>
          )}
        </div>
