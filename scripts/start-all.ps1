...
$healthy = $false
for ($i = 0; $i -lt 30; $i++) {
    $status = docker inspect --format='{{.State.Health.Status}}' printfilm-postgres 2>$null
...
pause
"@
$apiBat = Join-Path $env:TEMP "printfilm-start-api.bat"
...
pause
"@
$webBat = Join-Path $env:TEMP "printfilm-start-web.bat"
