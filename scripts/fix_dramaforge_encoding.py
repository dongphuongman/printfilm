...
TARGET = Path(__file__).resolve().parents[1] / (
    "services/api/src/main/java/com/printfilm/api/dramaforge/service/DramaForgeService.java"
)
...
def main() -> None:
    ...
    old_err = (
        ...
        '                            + "printfilm.tokenfree.default-tts-model ? elevenlabs/text-to-speech-turbo-2-5 ??????"\n'
        '                            + " ??: " + ex.getMessage(),'
    )
    new_err = (
        '                    "角色音色生成失败：TokenFree TTS 模型 " + ttsModel + " 调用出错。"\n'
        '                            + "请在 TokenFree 控制台确认已开通 ElevenLabs，并在 application-local.yml 设置 "\n'
        '                            + "printfilm.tokenfree.default-tts-model 为 elevenlabs/text-to-speech-turbo-2-5 等可用模型。"\n'
        '                            + " 详情: " + ex.getMessage(),'
    )
