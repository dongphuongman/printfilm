...
import com.printfilm.api.dramaforge.domain.*;
import com.printfilm.api.dramaforge.dto.DramaForgeDtos.*;
import com.printfilm.api.dramaforge.repository.*;
import com.printfilm.api.client.TokenFreeClient;
import com.printfilm.api.client.VolcengineTtsClient;
import com.printfilm.api.config.TokenFreeProperties;
import com.printfilm.api.domain.GenerationStatus;
import com.printfilm.api.domain.Project;
import com.printfilm.api.dto.CreateImageGenerationRequest;
import com.printfilm.api.dto.CreateVideoGenerationRequest;
...
import com.printfilm.api.security.UserPrincipal;
import com.printfilm.api.service.ImageGenerationService;
import com.printfilm.api.service.ProjectApiKeyResolver;
import com.printfilm.api.service.UploadStorageService;
import com.printfilm.api.service.VideoGenerationService;
...
public class DramaForgeService {
    ...
    public ConfigResponse updateConfig(UUID projectId, UpdateConfigRequest request) {
        ...
        if (request.imageQuality() != null && !request.imageQuality().isBlank()) {
            config.setImageQuality(com.printfilm.api.util.MediaSizeHelper.normalizeResolution(request.imageQuality()));
        }
        if (request.videoQuality() != null && !request.videoQuality().isBlank()) {
            config.setVideoQuality(com.printfilm.api.util.MediaSizeHelper.normalizeResolution(request.videoQuality()));
        }
     ...
     */
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
    public com.printfilm.api.dto.VideoGenerationResponse createVideoForProjectSuspended(
            UUID projectId,
            CreateVideoGenerationRequest request,
            String apiKeyHeader) {
        return videoGenerationService.createForProject(projectId, request, com.printfilm.api.config.ArkApiKeyContext.get());
    }

    private com.printfilm.api.dto.VideoGenerationResponse createShotVideoTask(
            UUID projectId,
            DramaForgeShot shot,
            DramaForgeConfig config,
            String prompt,
        ...
        for (var shot : shots) {
            ...
            try {
                var result = videoGenerationService.getForProject(
                        projectId, shot.getVideoJobId(), com.printfilm.api.config.ArkApiKeyContext.get());
                if (result.status() == GenerationStatus.COMPLETED) {
                    shot.setStatus(DramaForgeShotStatus.VIDEO_DONE);
                    shot.setErrorMessage(null);
                    synced++;
    ...
    private String resolveImageQuality(DramaForgeConfig config) {
        return com.printfilm.api.util.MediaSizeHelper.normalizeResolution(config.getImageQuality());
    }

    private String resolveVideoQuality(DramaForgeConfig config) {
        if (config.getVideoQuality() != null && !config.getVideoQuality().isBlank()) {
            return com.printfilm.api.util.MediaSizeHelper.normalizeResolution(config.getVideoQuality());
        }
        return "720p";
    }
